        ...
        try:
            ...
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    display_image = cv2.resize(image, (self.win_width, self.win_height))
                    display_image = cv2.cvtColor(display_image, cv2.COLOR_BGR2RGB)
                    self.screen.blit(pygame.image.frombuffer(display_image.tostring(), display_image.shape[1::-1], "RGB"),
                                (0, 0))
                    pygame.display.flip()
        finally:
